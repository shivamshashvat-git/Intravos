...
CREATE OR REPLACE VIEW v_trash_items WITH (security_invoker = on) AS
  ...
  UNION ALL
  SELECT 'customers', 'Customers', tenant_id, id, name, deleted_at FROM customers WHERE deleted_at IS NOT NULL
  UNION ALL
  SELECT 'quotations', 'Quotations', tenant_id, id, quote_number, deleted_at FROM quotations WHERE deleted_at IS NOT NULL;
